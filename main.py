...
from model.tmsgt_ddi import TMSGT_DDI
from torch_geometric.utils import degree
from torch.utils.data.distributed import DistributedSampler
from subgraph_extraction import smile_to_graph, read_smiles, read_interactions, generate_node_subgraphs, read_network
...
def init_args(user_args=None):
    ...
    parser = argparse.ArgumentParser(description='TMSGT_DDI')

    parser.add_argument('--model_name', type=str, default='TMSGT_DDI')
    ...
    parser.add_argument('--model_episodes', type=int, default=80)
    parser.add_argument('--extractor', type=str, default="MultiScaleExtractor")
    ...
    parser.add_argument('--local_max_hop', type=int, default=2,
                        help="[MultiScaleExtractor] Max hop distance for local sampling ")
    parser.add_argument('--nodes_per_scale', type=int, default=16,
                        help="[MultiScaleExtractor] Nodes to select per scale (local hop AND global).")
...
def load_data(args):
    ...
    print("load networks !!")
    num_node, network_edge_index, network_rel_index, num_rel = read_network(os.path.join(data_path, "networks.txt"))
    ...
    total_num_nodes = num_node + 1
    entity2id_path = os.path.join(data_path, "entity2id.txt")
    if os.path.exists(entity2id_path):
        with open(entity2id_path, 'r') as f:
            first_line = f.readline().strip()
            if len(first_line.split('\t')) == 1 and first_line.isdigit():
                total_num_nodes = max(total_num_nodes, int(first_line))

    print(f"Detected Total Nodes in KG: {total_num_nodes}")

    bert_path = os.path.join(data_path, "drug_features.npy")
    bkg_features = None

    if os.path.exists(bert_path):
        print(f"Loading BERT features from {bert_path}")
        drug_features = np.load(bert_path)
        feature_dim = drug_features.shape[1]

        print(f"Creating feature matrix with ZEROS for neighbor nodes...")
        full_features = np.zeros((total_num_nodes, feature_dim), dtype=np.float32)

        valid_rows = min(drug_features.shape[0], total_num_nodes)
        full_features[:valid_rows] = drug_features[:valid_rows]

        bkg_features = full_features
        print(f"Final Feature Matrix Shape: {bkg_features.shape}")
    else:
        print(f"Warning: BERT features NOT found. Falling back to all zeros (pure embedding learning).")
        bkg_features = np.zeros((total_num_nodes, 768), dtype=np.float32)
    ...
    print(data_sta)
    return interactions, labels, smile_graph, drug_subgraphs, data_sta, bkg_features
...
def init_model(args, dataset_statistics, bkg_feature_dim):
    if args.model_name == 'TMSGT_DDI':
        model = TMSGT_DDI(max_layer=args.layer,
                      num_features_drug = 67,
                      num_features_bkg=bkg_feature_dim,
...
def main(args = None, k_fold = 5):
    ...
    data, labels, smile_graph, node_graph, dataset_statistics, bkg_features = load_data(args)

    if bkg_features is not None:
        bkg_feature_dim = bkg_features.shape[1]
    else:

        bkg_feature_dim = 768
        print("Warning: No BKG features, model might fail if inputs are IDs.")
    ...
    for fold, (train_idx, test_idx, val_idx) in enumerate(zip(*split_fold(k_fold, data, labels, args.s_type))):
        ...
        test_loader = torch.utils.data.DataLoader(test_data, batch_size=args.batch_size, shuffle=True,
                                                  collate_fn=collate)
        eval_loader = torch.utils.data.DataLoader(eval_data, batch_size=args.batch_size, shuffle=True,
                                                  collate_fn=collate)

        if args.model_name:
            model, optimizer = init_model(args, dataset_statistics, bkg_feature_dim)
            model.to(device)
